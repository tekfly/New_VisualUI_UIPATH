...
# ---------------- Load JSON ----------------~
$jsonPath =".\json_files\product_versions.json"
$products = Get-Content $jsonPath -Raw | ConvertFrom-Json


# Fill ProductBox
$products.PSObject.Properties.Name | ForEach-Object {
    $ProductBox.Items.Add($_)
}

#Update VersionBox When product Change
$ProductBox.Add_SelectionChanged({
    ...
    if ($null -ne $selectedProduct) {

        $versions = $products.$selectedProduct.PSObject.Properties.Name
