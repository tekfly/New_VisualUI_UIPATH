...
# Features:
#   - Dynamic Main Products: loaded from product_versions.json
#   - Optional Extra Products: loaded from extra_products.json
#   - Add / Remove rows for both sections
#   - Unified download workflow
#   - Offline validation
#   - Installation screen with script generator + popup logger
# =====================================================================

# ---------- Ensure STA mode and elevation ----------
...
$global:ExtraProducts = Load-Json "extra_products.json"

# ---------- Helper: Folder dialog ----------
...
function New-ProductRow {
    ...
    if ($JsonSource) {
        $cbP.Items.Clear()
        $JsonSource.PSObject.Properties.Name | ForEach-Object {
            [void]$cbP.Items.Add($_)
        }
    }
...
$AddExtraPairBtn.Add_Click({
    if (-not $global:ExtraProducts) {
        Show-Error "extra_products.json not found or invalid."
        return
    }
...
function Test-Online {
    try {
        ...
        $req.Method="HEAD"; $req.Timeout=2000
...
$DownloadBtn.Add_Click({
    ...
    if ($urls.Count -eq 0) {
        Show-Error "No URLs found. Check your product/version selections or your JSON files."
        return
    }
...
function Get-SelectedFeatures {
    ...
    if ($RobotInstallCheck2.IsChecked) { $lst.Add("Robot") }
    if ($StudioInstallCheck2.IsChecked) { $lst.Add("Studio") }
    if ($ServiceModeCheck2.IsChecked) { $lst.Add("RegisterService") }
    return $lst
}
...
function Build-FullScript {
    ...
    $cmd = Build-MSICommand
    $log = '$env:TEMP\UiPathStudio_{0}.log' -f ([Guid]::NewGuid())
...
if (`$LASTEXITCODE -ne 0){
    throw "Installer exited with code `$LASTEXITCODE. Log: $log"
}
Write-Host "Install complete. Log: $log"
"@
}
...
$RunInstallerBtn.Add_Click({
    ...
    if (-not $global:InstallerRoot){ Show-Error "Installer not validated"; return }

    $msiCmd = Build-MSICommand
    $msiLog = Join-Path $env:TEMP ("UiPathStudio_{0}.log" -f ([Guid]::NewGuid()))
    $msiCmd += " /L*V `"$msiLog`""

    $popupXaml = New-InstallPopup
    [xml]$xml=$popupXaml
    ...
    $cancelBtn.Add_Click({
        try {
            ...
            Popup-Log $popup "Cancellation requested..."
    ...
    if ($exit -ne 0) {
        Popup-Log $popup "Installer exited with code $exit"
        Popup-Status $popup "Installation failed"
        Popup-Finish $popup
        return
    }
